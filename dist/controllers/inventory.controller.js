...
async function getInventoryLogs(req, res) {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Inventory Logs fetched", data: Inventorys });
  } catch (error) {
    console.error("Error fetching Inventorys:", error);
...
async function StockIn(req, res) {
  try {
    ...
    const result = await (0, inventory_service_1.stockInData)(
      inventoryId,
      itemId,
      changeQty,
    );
...
async function InventoryWastage(req, res) {
  try {
    ...
    const result = await (0, inventory_service_1.wastage)(
      inventoryId,
      itemId,
      changeQty,
      reason,
